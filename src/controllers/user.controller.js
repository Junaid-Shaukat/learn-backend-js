...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "server error while generating access and refresh token"
    );
  }
};
...
const updateUserAvatar = asyncHandler(async(req,res)=>{
 ...
 const user =  await User.findByIdAndUpdate(
  ...
  return res.status(200).jsone(new ApiResponse(200,user,"avatar updated successfully"))
})
const updateUserCoverImage = asyncHandler(async(req,res)=>{
  ...
  return res.status(200).jsone(new ApiResponse(200,user,"cover image updated successfully"))
})
export {
  registerUser,
  loginUser,
